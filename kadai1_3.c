...
int main(){
    ...
    double u_L = 0, u_R = 0;
    double a[N+2], b[N+1],c[N+1],alpha[N+2],beta[N+2],z[N+2],u[N+2],y[N+2];//クランクニコルソン法に必要な変数
    //初期化
    ...
    FILE *fp1_1_4;
    FILE *fp1_1_5;
    fp1_1_1= fopen("kadai1_3_1.txt", "w");
    fp1_1_2 = fopen("kadai1_3_2.txt", "w");
    fp1_1_3 = fopen("kadai1_3_3.txt", "w");
    fp1_1_4 = fopen("kadai1_3_4.txt", "w");
    fp1_1_5 = fopen("kadai1_3_5.txt", "w");
    ...
    for(int i=1; i<=step_num; i++){
        t += dt;
        //z を計算
        z[1] = (1-c_const)*u[1] + c_const*u_L + c_const*u[2]/2;
        for(int j=2; j<=N-1; j++){
            z[j] = (1-c_const)*u[j] + c_const*u[j-1]/2 + c_const*u[j+1]/2;
        }
        z[N] = (1-c_const)*u[N] + c_const*u_R + c_const*u[N-1]/2;
